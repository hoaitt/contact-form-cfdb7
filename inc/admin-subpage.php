...
class CFDB7_List_Table extends WP_List_Table
{
    ...
    private function table_data()
    {
        ...
        if ( ! empty($search) ) {
           ...
           $results = $wpdb->get_results( "SELECT * FROM $table_name WHERE  form_value LIKE '%$search%' ORDER BY form_date DESC LIMIT $start,20", OBJECT );
        }else{

            $results = $wpdb->get_results( "SELECT * FROM $table_name WHERE form_post_id = $form_post_id ORDER BY form_date DESC LIMIT $start,20", OBJECT );
        }
